...
let
  ...
  rustPkg = pkgs.lib.optionalAttrs (builtins.elem "rust" langs) {
    "${packageName}-rust" = rustProto {
  ...
  haskellPkg = pkgs.lib.optionalAttrs (builtins.elem "haskell" langs) {
    "${packageName}-haskell" = haskellProto {
      inherit
        src
        protos
        extraSources
        cabalBuildDepends
        useGoogleProtosFromHackage
        ;
  ...
  otherPkgs = {
    ...
    "${packageName}-src" = pkgs.lib.cleanSourceWith {
